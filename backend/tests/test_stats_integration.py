...
def test_complete_stats_system():
    ...
    assert (
        player.get_base_stat("atk") == original_base + 10
    ), "Base stat should be permanently modified"
    assert player.atk == original_base + 10, "Runtime should reflect base change"

    # Test temporary effect
    buff = StatEffect(
        "battle_buff", {"atk": 15, "max_hp": 50}, duration=3, source="spell"
    )
    ...
    assert (
        player.get_base_stat("atk") == original_base + 10
    ), "Base should not change from temporary effects"
    ...
    assert (
        final_atk == expected_final
    ), f"After cleanup, should have {expected_final} atk"
