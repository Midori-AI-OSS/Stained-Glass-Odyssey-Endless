...
def test_foe_scaling_cumulative_rooms():
    ...
    node2 = MapNode(
        room_id=1, room_type="battle-normal", floor=2, index=1, loop=1, pressure=0
    )
    ...
    assert (
        foe2.atk > foe1.atk
    ), f"Floor 2 Room 1 foe should be stronger than Floor 1 Room 1: {foe2.atk} > {foe1.atk}"
    assert (
        foe2.max_hp > foe1.max_hp
    ), f"Floor 2 Room 1 foe should have more HP than Floor 1 Room 1: {foe2.max_hp} > {foe1.max_hp}"
...
def test_foe_scaling_room_progression_within_floor():
    ...
    # Room 5 Floor 1
    node1 = MapNode(
        room_id=5, room_type="battle-normal", floor=1, index=5, loop=1, pressure=0
    )

    # Room 10 Floor 1
    node2 = MapNode(
        room_id=10, room_type="battle-normal", floor=1, index=10, loop=1, pressure=0
    )
    ...
    assert (
        foe2.atk > foe1.atk
    ), f"Room 10 foe should be stronger than Room 5: {foe2.atk} > {foe1.atk}"
...
def test_foe_level_cumulative_progression():
    ...
    node1 = MapNode(
        room_id=45, room_type="battle-normal", floor=1, index=45, loop=1, pressure=0
    )
    node2 = MapNode(
        room_id=1, room_type="battle-normal", floor=2, index=1, loop=1, pressure=0
    )
...
def test_cumulative_room_calculation():
    ...
    assert (
        cumulative1 == 1
    ), f"Floor 1 Room 1 should be cumulative room 1, got {cumulative1}"
    ...
    assert (
        cumulative2 == 46
    ), f"Floor 2 Room 1 should be cumulative room 46, got {cumulative2}"

    # Floor 2, Room 10 should be cumulative room 55 (45 + 10)
    node3 = MapNode(
        room_id=10, room_type="battle-normal", floor=2, index=10, loop=1, pressure=0
    )
    cumulative3 = (node3.floor - 1) * MapGenerator.rooms_per_floor + node3.index
    assert (
        cumulative3 == 55
    ), f"Floor 2 Room 10 should be cumulative room 55, got {cumulative3}"
