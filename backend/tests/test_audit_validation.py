...
class TestCardValidation:
    ...
    def test_vital_surge_effects(self):
        ...
        assert card.stars == 2, "Vital Surge should be a 2-star card"
...
class TestCharacterValidation:
    ...
    def test_luna_character_definition(self):
        ...
        # Test basic class attributes that are directly accessible
        assert Luna.id == "luna"
        assert Luna.name == "Luna"
        # Remove this line - passives are in field annotations
        assert Luna.actions_display == "number"

    def test_luna_damage_type(self):
        """Test Luna has proper damage type assignment."""
        # Test that Luna class has damage_type field annotation
        assert hasattr(Luna, '__annotations__')
        assert 'damage_type' in Luna.__annotations__
