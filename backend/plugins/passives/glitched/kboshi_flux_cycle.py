...
@dataclass
class KboshiFluxCycleGlitched(KboshiFluxCycle):
    """Glitched variant of Kboshi's Flux Cycle with doubled effects."""
    plugin_type = "passive"
    ...
    async def apply(self, target) -> None:
        """Apply Glitched Flux Cycle element switching mechanics for Kboshi."""
        import random
        ...
        # Glitched: Same chance to switch (80%)
        if random.random() < 0.8:
            ...
            if self._damage_stacks[entity_id] > 0 or self._hot_stacks[entity_id] > 0:
                ...
                # Glitched: Apply doubled mitigation debuff (-4% per stack instead of -2%)
                if stacks > 0:
                    mitigation = stacks * -0.04
            ...
            # Glitched: Apply doubled HoT - heal 10% max HP per stack (doubled from 5%)
    ...
    def get_description(cls) -> str:
        return (
            ...
            "Failing to switch grants a 40% attack bonus (doubled) and a doubled HoT for that turn; "
            "switching clears stacks and applies -4% mitigation to foes per stack (doubled)."
        )
