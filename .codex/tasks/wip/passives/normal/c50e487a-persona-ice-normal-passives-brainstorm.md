# Task: Brainstorm Normal Tier Tagged Passives for PersonaIce

## Background
PersonaIce's core moveset and lore live in `backend/plugins/characters/persona_ice.py`. We still need baseline tagged passive concepts used when no encounter modifiers are present so future Tier-Normal work has a clear head start. This is an ideation task only—capture concepts Coders can later formalize inside `backend/plugins/passives/normal/persona_ice.py`.
...
##All Tiers: **Normal: Cryo Cycle (IMPLEMENTED)** - Freeze mechanics, ice armor, frost accumulation | **Prime: Absolute Zero** - Permanent freeze zones, ice mastery, "Glacial Epoch" ultimate | **Boss: Eternal Winter** - **P:** Chill→Frost→Blizzard→Absolute zero | **Glitched: Temperature Glitch** - Freeze/thaw random, ice damages allies, cold inverts to heat
