# Task: Brainstorm Normal Tier Tagged Passives for LadyOfFire

## Background
LadyOfFire's core moveset and lore live in `backend/plugins/characters/lady_of_fire.py`. We still need baseline tagged passive concepts used when no encounter modifiers are present so future Tier-Normal work has a clear head start. This is an ideation task only—capture concepts Coders can later formalize inside `backend/plugins/passives/normal/lady_of_fire.py`.
