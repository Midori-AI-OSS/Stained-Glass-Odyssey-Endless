# Task: Brainstorm Glitched Tier Tagged Passives for LadyStorm

## Background
LadyStorm's core moveset and lore live in `backend/plugins/characters/lady_storm.py`. We still need glitched-tag passives that warp the character's behavior in unpredictable or corrupted ways so future Tier-Glitched work has a clear head start. This is an ideation task only—capture concepts Coders can later formalize inside `backend/plugins/passives/glitched/lady_storm.py`.
...
##Glitched: Weather Malfunction - Storms target wrong zones, weather inverts (rain=fire), pressure corrupted (vacuum/crush zones random), tornado paths unpredictable, atmospheric data impossible
