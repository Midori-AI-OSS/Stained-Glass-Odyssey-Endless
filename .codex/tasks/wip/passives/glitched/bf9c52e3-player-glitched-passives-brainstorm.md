# Task: Brainstorm Glitched Tier Tagged Passives for Player

## Background
Player's core moveset and lore live in `backend/plugins/characters/player.py`. We still need glitched-tag passives that warp the character's behavior in unpredictable or corrupted ways so future Tier-Glitched work has a clear head start. This is an ideation task only—capture concepts Coders can later formalize inside `backend/plugins/passives/glitched/player.py`.
...
##Glitched: XP Corruption - Level values fluctuate randomly, XP can become negative, stat gains inverted (lose stats on level up), progression system broken, level display corrupted
