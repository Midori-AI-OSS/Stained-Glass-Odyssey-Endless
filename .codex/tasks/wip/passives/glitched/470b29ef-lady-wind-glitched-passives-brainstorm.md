# Task: Brainstorm Glitched Tier Tagged Passives for LadyWind

## Background
LadyWind's core moveset and lore live in `backend/plugins/characters/lady_wind.py`. We still need glitched-tag passives that warp the character's behavior in unpredictable or corrupted ways so future Tier-Glitched work has a clear head start. This is an ideation task only—capture concepts Coders can later formalize inside `backend/plugins/passives/glitched/lady_wind.py`.
...
##Glitched: Wind Chaos - Shields push allies away, air currents unpredictable/chaotic, wind direction inverts randomly, mobility buffs become debuffs, flight control lost
