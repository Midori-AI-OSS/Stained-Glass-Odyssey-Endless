# Task: Brainstorm Glitched Tier Tagged Passives for PersonaIce

## Background
PersonaIce's core moveset and lore live in `backend/plugins/characters/persona_ice.py`. We still need glitched-tag passives that warp the character's behavior in unpredictable or corrupted ways so future Tier-Glitched work has a clear head start. This is an ideation task only—capture concepts Coders can later formalize inside `backend/plugins/passives/glitched/persona_ice.py`.
...
##Glitched: Temperature Glitch - Freeze/thaw cycles random and rapid, ice damages allies, cold inverts to heat, frost accumulation corrupted, armor shatters unpredictably
