# Task: Brainstorm Glitched Tier Tagged Passives for PersonaLightAndDark

## Background
PersonaLightAndDark's core moveset and lore live in `backend/plugins/characters/persona_light_and_dark.py`. We still need glitched-tag passives that warp the character's behavior in unpredictable or corrupted ways so future Tier-Glitched work has a clear head start. This is an ideation task only—capture concepts Coders can later formalize inside `backend/plugins/passives/glitched/persona_light_and_dark.py`.
