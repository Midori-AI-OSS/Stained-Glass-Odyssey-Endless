# Task: Brainstorm Glitched Tier Tagged Passives for LadyFireAndIce

## Background
LadyFireAndIce's core moveset and lore live in `backend/plugins/characters/lady_fire_and_ice.py`. We still need glitched-tag passives that warp the character's behavior in unpredictable or corrupted ways so future Tier-Glitched work has a clear head start. This is an ideation task only—capture concepts Coders can later formalize inside `backend/plugins/passives/glitched/lady_fire_and_ice.py`.
