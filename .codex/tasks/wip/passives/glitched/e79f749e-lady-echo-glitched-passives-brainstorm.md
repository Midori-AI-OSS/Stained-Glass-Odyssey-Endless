# Task: Brainstorm Glitched Tier Tagged Passives for LadyEcho

## Background
LadyEcho's core moveset and lore live in `backend/plugins/characters/lady_echo.py`. We still need glitched-tag passives that warp the character's behavior in unpredictable or corrupted ways so future Tier-Glitched work has a clear head start. This is an ideation task only—capture concepts Coders can later formalize inside `backend/plugins/passives/glitched/lady_echo.py`.
...
## Glitched: Feedback Loop - Infinite echo recursion (stack overflow), sound inverts (silence=damage, noise=heal), resonance corrupted (wrong targets), static noise replaces effects, echoes become autonomous hostile entities
