# Task: Brainstorm Prime Tier Tagged Passives for LadyStorm

## Background
LadyStorm's core moveset and lore live in `backend/plugins/characters/lady_storm.py`. We still need prime-tag passives that feel like exalted upgrades layered on the base kit so future Tier-Prime work has a clear head start. This is an ideation task only—capture concepts Coders can later formalize inside `backend/plugins/passives/prime/lady_storm.py`.
...
##Prime/Boss/Glitched: **Prime: Cataclysmic Tempest** - Multi-storms, weather absolute control | **Boss: Atmospheric Collapse** - **P:** Breeze→Gale→Hurricane→Planetary | **Glitched: Weather Malfunction** - Storms misdirected, weather inverted, pressure corrupted
