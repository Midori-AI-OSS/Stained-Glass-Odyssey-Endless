# Task: Brainstorm Prime Tier Tagged Passives for PersonaLightAndDark

## Background
PersonaLightAndDark's core moveset and lore live in `backend/plugins/characters/persona_light_and_dark.py`. We still need prime-tag passives that feel like exalted upgrades layered on the base kit so future Tier-Prime work has a clear head start. This is an ideation task only—capture concepts Coders can later formalize inside `backend/plugins/passives/prime/persona_light_and_dark.py`.
...
##Prime/Boss/Glitched: **Prime: Perfect Equilibrium** - Both forms active, balance mastered, fusion state | **Boss: Eternal Duality** - **P:** Balance→Imbalance→Chaos→Merge | **Glitched: Form Corruption** - Forms blend wrong, balance broken, shifts uncontrolled
