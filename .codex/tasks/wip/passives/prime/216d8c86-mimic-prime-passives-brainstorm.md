# Task: Brainstorm Prime Tier Tagged Passives for Mimic

## Background
Mimic's core moveset and lore live in `backend/plugins/characters/mimic.py`. We still need prime-tag passives that feel like exalted upgrades layered on the base kit so future Tier-Prime work has a clear head start. This is an ideation task only—capture concepts Coders can later formalize inside `backend/plugins/passives/prime/mimic.py`.
