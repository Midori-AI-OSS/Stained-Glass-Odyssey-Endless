# Task: Brainstorm Prime Tier Tagged Passives for LadyWind

## Background
LadyWind's core moveset and lore live in `backend/plugins/characters/lady_wind.py`. We still need prime-tag passives that feel like exalted upgrades layered on the base kit so future Tier-Prime work has a clear head start. This is an ideation task only—capture concepts Coders can later formalize inside `backend/plugins/passives/prime/lady_wind.py`.
