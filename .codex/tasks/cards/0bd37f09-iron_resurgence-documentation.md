# Add Documentation Fields to Iron Resurgence Card

## Objective

Add `full_about` and `summarized_about` fields to the Iron Resurgence card plugin to support enhanced documentation and future AI integration.
...
**File to modify:** `backend/plugins/cards/iron_resurgence.py`
...
```python
@dataclass
class IronResurgenceCard(CardBase):
...
✅ `full_about` includes all specific mechanics (+200% DEF, +200% HP, 10% HP revive, 4-turn cooldown)
✅ Descriptions accurately match code implementation (lines 15-19, 27-31)
