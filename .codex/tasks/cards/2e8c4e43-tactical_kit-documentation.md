# Add Documentation Fields to Tactical Kit Card

## Objective

Add `full_about` and `summarized_about` fields to the Tactical Kit card plugin to support enhanced documentation and future AI integration.
...
**File to modify:** `backend/plugins/cards/tactical_kit.py`
...
```python
@dataclass
class TacticalKitCard(CardBase):
    id: str = "tactical_kit"
...
✅ `full_about` includes all specific mechanics (+2% ATK/HP, 1% HP conversion, +2% ATK boost, once per battle)
✅ Descriptions accurately match code implementation (lines 15-19, 42, 56)
