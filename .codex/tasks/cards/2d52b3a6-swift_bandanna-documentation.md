# Add Documentation Fields to Swift Bandanna Card

## Objective

Add `full_about` and `summarized_about` fields to the Swift Bandanna card plugin to support enhanced documentation and future AI integration.
...
**File to modify:** `backend/plugins/cards/swift_bandanna.py`
...
```python
@dataclass
class SwiftBandannaCard(CardBase):
...
Audited by reviewing `backend/plugins/cards/swift_bandanna.py`. All acceptance criteria met:
- ✓ Old `about` field removed (confirmed absent)
- ✓ `full_about` properly implemented: "+3% Crit Rate & +3% Dodge Odds; on dodge, gain +1% crit rate for next action"
- ✓ `summarized_about` properly implemented: "Boosts crit rate and dodge odds; dodging grants bonus crit rate"
- ✓ Description format standards followed (summarized has no numbers, full has "+3%", "+1%")
- ✓ Descriptions accurate to dodge-triggered crit rate mechanics
- ✓ Code style consistent with repository conventions
- ✓ Proper temporary buff implementation
