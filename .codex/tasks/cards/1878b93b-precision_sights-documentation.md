# Add Documentation Fields to Precision Sights Card

## Objective

Add `full_about` and `summarized_about` fields to the Precision Sights card plugin to support enhanced documentation and future AI integration.
...
**File to modify:** `backend/plugins/cards/precision_sights.py`
...
```python
@dataclass
class PrecisionSightsCard(CardBase):
...
✅ `full_about` includes all specific mechanics (+4% Crit Damage base, +2% per crit, 2 turns, stacking)
✅ Descriptions accurately match code implementation (lines 15-19, 41)
