# Add Documentation Fields to Wooden Idol Relic

## Objective

Add `full_about` and `summarized_about` fields to the Wooden Idol relic plugin to support enhanced documentation and future AI integration.
...
**File to modify:** `backend/plugins/relics/wooden_idol.py`
...
```python
@dataclass
class WoodenIdolRelic(RelicBase):
    id: str = "wooden_idol"
...
✅ `full_about` includes all specific mechanics (+3% Effect Res per stack, +1% Effect Res bonus per stack, 1 turn)
✅ Stacking behavior properly documented (multiplicative base effect, additive bonus)
