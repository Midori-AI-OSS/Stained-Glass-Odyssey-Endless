# Add Documentation Fields to Killer Instinct Relic

## Objective

Add `full_about` and `summarized_about` fields to the Killer Instinct relic plugin to support enhanced documentation and future AI integration.
...
**File to modify:** `backend/plugins/relics/killer_instinct.py`
...
```python
@dataclass
class KillerInstinctRelic(RelicBase):
...
✅ `full_about` includes all specific mechanics (+75% ATK per stack, +50% SPD per stack, duration info)
