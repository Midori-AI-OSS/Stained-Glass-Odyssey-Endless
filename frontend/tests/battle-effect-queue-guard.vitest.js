...
describe('BattleView effect queue guard', () => {
  ...
  it('stabilizes polling when Luna swords spawn alongside hit events', async () => {
    const warnSpy = vi.spyOn(console, 'warn').mockImplementation(() => {});
    const deferred = (() => {
      let resolve;
      const promise = new Promise((res) => {
    ...
    deferred.resolve({
      party: [
        { id: 'hero', hp: 40, max_hp: 100 },
      ],
      foes: [
        { id: 'luna-boss', hp: 500, max_hp: 500 },
      ],
      foe_summons: [
        {
          owner_id: 'luna-boss',
      ...
      recent_events: [
        {
          type: 'damage_taken',
          amount: 18,
          source_id: 'luna-sword-alpha',
          target_id: 'hero',
          metadata: {
            source_name: 'Luna',
            owner_id: 'luna-boss',
            attack_sequence: 'alpha-1',
            target_id: 'hero',
          },
        },
      ],
